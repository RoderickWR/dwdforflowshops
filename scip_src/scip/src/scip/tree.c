...
/** set ineq sign of constraint that is added to that node as stored info at that node*/
void SCIPnodeSetIneqSign(
   SCIP_NODE*            node,                /**< node */
   int sign // 1 means less than 0 means greater than
   )
{
   assert(node != NULL);
...
/** get ineq sign of constraint that is added to that node as stored info at that node*/
int SCIPnodeGetIneqSign(
   SCIP_NODE*            node                /**< node */
   )
{
   assert(node != NULL);

   return (int) node->sign;
}

void SCIPnodeSetMIdx(SCIP_NODE* node, int mIdx) {
   assert(node != NULL);
   node->mIdx = mIdx;
}

int SCIPnodeGetMIdx(SCIP_NODE* node) {
   assert(node != NULL);
   return (int) node->mIdx;
}

/** gets the id1 of the node */
int SCIPnodeGetId1(
   SCIP_NODE*            node                /**< node */
   )
{
